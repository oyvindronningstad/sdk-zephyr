    ...
    for x in reqs.keys():
        if 'inside' in reqs[x].keys():
            ...
            if not reqs[x]['inside']:
                del reqs[x]['inside']
        if 'inherit_size' in reqs[x].keys():
            [remove_item_not_in_list(reqs[x]['inherit_size'], reqs.keys())]
            if not reqs[x]['inherit_size']:
                del reqs[x]['inherit_size']
...
def load_size_config(adr_map):
    ...
    for k, v in adr_map.items():
        if 'size' not in v.keys() and 'span' not in v.keys() and k != 'app' and 'inherit_size' not in v.keys():
...
def inherited_size(reqs, inherit_from, total_size):
    inheritor_count = reqs[inherit_from]['inheritors']
    return int(sizeof(reqs, inherit_from, total_size) / (inheritor_count + 1))


def get_size_source(reqs, inheritor):
    size_source = inheritor
    while 'inherit_size' in reqs[size_source].keys():
        # Find "original" source.
        size_source = reqs[size_source]['inherit_size'][0]
    return size_source
...
def set_inherited_size(reqs, sub_partitions, total_size, region):
    ...
    for req in all_reqs:
        if 'inherit_size' in all_reqs[req].keys():
            ...
            if 'inheritors' not in all_reqs[size_source].keys():
                all_reqs[size_source]['inheritors'] = 0
            all_reqs[size_source]['inheritors'] += 1
            all_reqs[req]['inherit_size'] = [size_source]
    new_sizes = dict()
    for req in all_reqs:
        if 'inherit_size' in all_reqs[req].keys():
            new_sizes[req] = inherited_size(all_reqs, all_reqs[req]['inherit_size'][0], total_size)
